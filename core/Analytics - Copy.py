...
class FrameProcessor:
    def __init__(self, frame_queue,obj_db,obj_ppe_detector:PPEDetectionModule=None,frame_lock=None):
        try:
            ...
            prev_id=0
            self.obj_ppe_detector= obj_ppe_detector
        ...
        while self.running:
            ...
            try:
                ...
                try:
                    ...
                    self.obj_ppe_detector.ppe_detection_input_queue.put(
                        (frame, person_id, bbox, cam_id, conf, self.person_detections),
                        block=False
                    )
                except queue.Full:
                    # Queue is full — drop the oldest frame to make room
                    try:
                        self.obj_ppe_detector.ppe_detection_input_queue.get_nowait()  # remove oldest
                        self.obj_ppe_detector.ppe_detection_input_queue.put_nowait(
                            (frame, person_id, bbox, cam_id, conf, self.person_detections)
                        )
                    except queue.Empty:
                        pass
               
                # self.obj_ppe_detector.ppe_detection_input_queue.put((frame,person_id,bbox,cam_id,conf,self.person_detections),block=False)
                
                detection_result = self.person_detections.get(person_id, timeout=0.1)
                ...
                #         self.person_detections[person_id] = detection_result
                    ...
                    # Insert event ONLY after 5–6 same frames
                    # if (
                    #     self.same_status_count[person_id] >= self.MIN_STABLE_FRAMES
                    #     and detection_result["needs_warning"]
                    # ):
                # try:
                #     event_id_str = f"{person_id}"
                #     self.event_queue.put(
                #         (frame, person_id, cam_id, detection_result, bbox, event_id_str),
                #         block=False
                #     )

                #         # Prevent repeated inserts for same violation
                #         # self.same_status_count[person_id] = 0
...
#_______________________________________________________________________________draw_visualizations_____________________________________________________________
    ...
    def draw_visualizations(self, frame, person_id, bbox, cam_id):
        ...
        try:
            ...
            status_text = f"ID:{person_id} - N/A"
            ...
            if detection:
                ...
                if safety_status != "unknown":
                    if needs_warning:
                        main_color = self.colors['unsafe']
                        status_text = f"ID:{person_id} - {safety_status.replace('_', ' ').title()}"
                    else:
                        ...
                        with torch.no_grad():
                            ...
                            if results and results[0].boxes is not None:
                                ...
                                for box in results[0].boxes:
                                    ...
                                    if person_id in self.time_stamp_track_id_records:
                                        person_id = self.time_stamp_track_id_records[person_id]
                                    else:
                                        self.time_stamp_track_id_records[person_id]=f"{cam_id}_{track_id}_{int(time.time())}"
