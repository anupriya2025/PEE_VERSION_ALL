            ...
            try:
                ...
                #         # First time seeing this person
                    ...
                    if current_status == last_status:
                        self.same_status_count[person_id] = min(self.same_status_count[person_id]+1 , 30 )
                        last_status = None
                        try:
                            last_status=  self.last_inserted_status[person_id]
                            if last_status:
                                if last_status[0] != current_status[0] or last_status[1] != current_status[1] or last_status[2] != current_status[2] :
                                    try:
    ...
    def _perform_ppe_detection(self, frame, person_id, bbox, cam_id, conf):
        ...
        try:
            ...
            if detection:
                ...
                if time.time()- last_detection_time < time_gaph_needed and self.same_status_count[person_id] >=  30:
                    return None
            with torch.no_grad():
